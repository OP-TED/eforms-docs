...
DRAFT, SUBMITTED, STOPPED, PUBLISHED, DELETED, NOT_PUBLISHED, ARCHIVED, VALIDATION_FAILED.
...
The CVS will give a validation report, which will detail all the reasons why a notice did not get through and the status “NOT_PUBLISHED” 
...
in the user interface and when querying the API with status “validation failed”. The same businessID (noticeID + versionID) cannot be reused. 
