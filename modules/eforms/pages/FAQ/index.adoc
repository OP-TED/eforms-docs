...
+
The Notice VersionID is described in the link:https://docs.ted.europa.eu/eforms/0.7.0/schema/elements.html#noticeIDSection[Notice & Version Identifiers] 
...
+ 
In the link:https://docs.ted.europa.eu/eforms/0.7.0/schema/change-notice.html[Change Notice] section, the word "version" is used 
...
available as long as they are supported by the Legislation, allowing for more flexibility on the timing of upgrades on the eSenders’ applications. 
Updating applications to use new versions of the SDK should require minimal effort if the applications are built to integrate the SDK components.
+
...
+
The European Commission is planning to publish an amendment to the 2019 eForms implementing regulations. 
This amendment will change and add several business terms. 
...
We have a specific definition for link:https://docs.ted.europa.eu/eforms/0.5.0/fields/index.html#_what_is_a_field[Fields].
...
+
The latest release of the TED XML to eForms Converter (TEDXDC) has been published on link:https://github.com/OP-TED/ted-xml-data-converter[GitHub]. 
This release of the tool can partially convert Contract Notices published using the R.2.0.9 TED XML schema. 
...
see https://docs.ted.europa.eu/eforms/0.7.0/schema/parties.html#organizationSection. It has a single identifier, 
which must follow the pattern "ORG-XXXX", where "XXXX" is four digits. The first organisation must have identifier "ORG-0001", 
the second one "ORG-0002", etc. A numerical sequence in numbering does not have to be observed.  
+
An organization might have several contact details, each for one or more different functions. Each contact is defined in a TouchPoint, 
which has an identifier following the pattern "TPO-XXXX". An example XML for a Buyer is shown in: 
https://docs.ted.europa.eu/eforms/0.7.0/schema/parties.html#buyerSection. 
+
...
https://docs.ted.europa.eu/eforms/0.7.0/schema/parties.html#_legislation_information_provider 
and the following section: https://docs.ted.europa.eu/eforms/0.7.0/schema/parties.html#_other_rolessubroles  
+
These references use fields OPT-300 and OPT-301. These and other similar references are listed in: 
https://docs.ted.europa.eu/eforms/0.7.0/schema/identifiers.html 
...
link:https://docs.ted.europa.eu/eforms/0.7.0/schema/identifiers.html#_referring_to_objects[IDs & References].
+
...
+
- Some codelists are "technical" codelists that are required only because of the use of UBL to implement eForms. 
The "conditions" codelist for BT-70 is an example. These will not be published on the EU Vocabularies Authority tables page. 
+
- Some codelists are made available first in the SDK on GitHub, because the process for publishing them 
on the EU Vocabularies Authority tables page takes longer. 
+
For more information, see https://docs.ted.europa.eu/eforms/0.7.0/codelists/index.html[Tailored Codelists] 
...
BT-755-Lot, BT-772-Lot and BT-777-Lot all reference codelistes in the“xPathAbsolute”/”xPathRelative” field, have a “type”-attribute called “text-multilingual” and a “legal-type”-attribute called “TEXT” and therefore a codelist is not attached to these fields. All those codelists are at least referenced in the “xPathAbsolute” field. How are these fields validated against the codelists?::
...
Why is the codelist for BT-715/BT-716/BT-725 (green-procurement) not present either on the EU vocabularies Authority tables page or in the "Codelist" folder on GitHub?:: 

The Commission is preparing some changes to the eForms Regulation Annex table 2, to be published and implemented before November 2022. 
Some of these changes affect Green Procurement and may affect the "green-procurement" codelist. We will publish this codelist as soon as it has been finalised. 
