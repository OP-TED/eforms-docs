...
Notices submitted to Preview eNotices2 as of 8 September 2023 did not acquire status “published” due to an error in the export process. The export is now working, the status published is sent back and the root cause of the current issue has been identified. You might still encounter some delays in getting the status published until the final fix will be available in the next release of the application but a workaround is in place.

The following issues have been identified as of application version 1.8.3:

* In subtypes "17", "18", "22" when OPT-060-Lot rule is triggered, users must fill in BT-70-Lot. 
...
* Feature “Add a new language” and using the automatic translation service to prefill your new notice linguistic version is currently not working. 

The following issues have been identified as of application version 1.11.1:

* Legal basis (BT-01(f)-Procedure) field cannot be repeated.
...
Due to an error in our internal systems, notices submitted to eNotices2 Preview from 6 to 8 November 2023 did not get status Published; the issue was fixed on 8 November.
...
==== Tips for eSenders
    ...
    * eNotices2 API supports currently SDK versions 1.3, 1.4, 1.5, 1.6, 1.7, 1.8 and 1.9. SDK version 1.9 is supported in Preview environment as of 10 October 2023 and in Production as of 11 October 2023. For the value to indicate in the cbc:CustomizationID element, it should always have the format "eforms-sdk-major.minor".  See this page for more details: 
...
==== Planned updates 
[horizontal]
Indicative planning:: November 2023
...
NOTE: The current application version of Preview is into Production as of 8 November 2023.
...
SDK versions:: 1.0.0, 1.1.1, 1.2.1, 1.3.4, 1.4.3, 1.5.3, 1.6.1, 1.7.0, 1.8.0, 1.9.0, 1.10.0
...
NOTE: The validation mode "dynamic" checks data that may vary in time, e.g. the current date or information in another notice. As of SDK 1.9 a dynamic rule was added which checks that a notice ID does not match a notice ID that has already been published on TED. You can find the SDK Active versions per environment at: https://docs.ted.europa.eu/home/eforms/active-versions/index.html
...
SDK versions:: 1.0.0, 1.1.1, 1.2.1, 1.3.2, 1.4.2, 1.5.0, 1.5.1, 1.6.0, 1.7.0, 1.8.0, 1.9.0
