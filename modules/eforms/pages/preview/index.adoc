...
SDK versions:: 1.3.2, 1.4.1, 1.5.0, 1.5.1, 1.6.0
...
* Currently, it is not possible to refer to a TED-XML notice (using nnnnnn-yyyy publication number in BT-758) in a Change notice; a Change notice can only refer to an eForms notice ID (uuid) in BT-758. An issue in our back-office system (TED Monitor) will reject such notices causing them to end up in status "Not Published”. The workaround is to refer to the original notice in the additional information field.  We are working to fix this bug by July 2023. 
...
NOTE: The current application version of Preview goes into Production on 21 May 2023.
