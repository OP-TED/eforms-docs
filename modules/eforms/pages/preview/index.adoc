...
The application also allows users to act upon notices and procedures, archiving, renaming and transferring them to other users or other procedures as well as import/ export notices in XML. Import function is partially supported.   
...
All validation errors are currently displayed as a pop-up window, without pointing to the error location. 
...
Apart from the Organisation Name, Organisation Identifier and Oganisation Part Name, the rest of My Form Settings is currently not fully implemented and this function should not be used. Currently Touchpoints and Groups of Lots cannot be deleted once created.
