...
The 'validationReportUrl' is currently not fully correct; /api/v2/ is missing in the URL. Until this bug is fixed, eSenders should use instead URL pattern 'https://enotices2.preview.ted.europa.eu/esenders/api/v2/notice/notices/{businessId}/validation-report' where the {businessId} is replaced with value taken from the submitted notice ('noticeId-versionId') or with {businessId} from validationReportUrl.
...
==== Tips for eSenders
    ...
    * To avoid authorisation issues when using eNotices2 API, make sure you generate your API key in the corresponding environment of the TED Developer Portal. Then log in at least once in the corresponding environment of the User Interface to pair your API key with your eNotices2 account:  
      ** link:https://developer.ted.europa.eu/home[Developer Portal in Production] 
      ** link:https://developer.preview.ted.europa.eu/home[Developer Portal in Preview]
    * For the value to indicate in the cbc:CustomizationID element, it should always have the format "eforms-sdk-major.minor".  See this page for more details: 
