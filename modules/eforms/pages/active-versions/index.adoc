...
SDK expiration date::
  ...
  The expiration date of an SDK version may be extended beyond the standard lifespan of twelve (12) months (see "SDK extended lifespan" below).
Active SDK::
  A version of the SDK is said to be "active" during the period that TEDCVS accepts notices based on it. This period is defined by the SDK release date and the SDK expiration date.
...
Deprecated SDK::
  An SDK version is said to be "deprecated" after its expiration date has been reached. After this date, TEDCVS will no longer accept notices created using that version of the SDK.
