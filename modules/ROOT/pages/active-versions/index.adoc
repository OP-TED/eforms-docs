...
TED API provides information on the set of active SDK versions in JSON format through the xref:api:ROOT:developer-operations.adoc#version-range[version-range operation] of the xref:api:ROOT:developer-operations.adoc[Developer Operations API].
