...
https://docs.ted.europa.eu/api/latest/index.html[Read more about TED API]
...
- transmittedAt: Records the dispatch date of the notice as per BT-803 (Notice Dispatch Date eSender) in UTC (e.g., 2022-07-25T14:30:00Z); this will be "null" if not provided and is only for future notices.
  * eSenders are encouraged to transition to TEDAPI v3 by the end of September 2025 and read the https://docs.ted.europa.eu/api/latest/index.html[revised TED API Docs].
...
NOTE: Current application version is in Production as of 20 March 2025.
