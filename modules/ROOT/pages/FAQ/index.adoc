...
* 2024-03-21 - link:https://www.youtube.com/watch?v=Msw9w9-RwTE
...
+
Users will also be able to render eForms notices in HTML or PDF using the service provided by TED Viewer 2022, which is available through an API.
...
+
The lifespan of the various SDK version is documented on the link:https://docs.ted.europa.eu/eforms-common/active-versions/index.html[Active SDK versions] page, however, eSenders should always consult the information provided by the TED API. 
...
+
OP has also developed an application to convert from TED XML to eForms XML using these XSLT files and which will is provided as a public API service. 
+
...
of the User Interface to pair their API key with their eNotices2 account. To avoid authientication issues after generating a key, eSenders should perform at least one valid API request to eNotices2 with their key.
+
