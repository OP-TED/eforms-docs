...
+
We are currently preparing the schedule for the 2026 workshops and will update our link:https://op.europa.eu/en/web/ted-eforms/home[events page] in early 2026.
...
+
Authorised applications can send complete notices to the API data interface of eNotices2 (https://docs.ted.europa.eu/api/latest/publish.html[Publication API]), 
...
+
eSenders should create instead a “profile” in the https://developer.ted.europa.eu/home[TED Developer Portal] using a *functional email address* in order to generate an API key. 
...
The https://ted.europa.eu/en/release-calendar[release calendar] on TED includes the dates on which there will be an issue of the OJ S. 
...
+
* TED eSender workshops website: includes agendas, presentations, Q&A summaries, and other materials from all eForms-related workshops + 
...
An overview of the 
link:https://op.europa.eu/documents/11465927/11661400/2023-01-02-Lifecycle+of+eForms+notices-3rd+eForms+Technical+Workshop.pptx/a83fc6b8-191e-3e20-a412-7b94ba5317cc?t=1675250338281[lifecycle of eForms notices] was presented 
during the https://op.europa.eu/en/web/ted-eforms/agenda[3rd eForms Technical Workshop].
+
The lifecycle is represented by statuses such as DRAFT, SUBMITTED, PUBLISHED, PUBLISHING, STOPPED, NOT PUBLISHED, DELETED, ARCHIVED, and VALIDATION FAILED. Details on each status can be found throughout this FAQ; try searching by status name.
+
See also section "Understanding the Notice Workflow" in our https://docs.ted.europa.eu/api/latest/index.html#_tips_for_esenders[Tips for eSenders].
...
A Change notice is a reproduction of its parent notice with an extra section to advertise corrections or updates to the procurement or procurement documents.
+
A Change form can only be used once the parent notice has been published, to avoid conflicts between multiple edits.
If the parent notice has not yet been published, users can stop publication and resubmit instead.
+
If a notice contains multiple clerical errors, it can be cancelled, making it null and void; note that this does not cancel the procedure itself.
To cancel the entire procedure, a Result notice (CAN) must be published with “No winner chosen” and the corresponding non-award reason, regardless of whether the submission deadlines have been reached or not.
+
As a rule, major structural changes such as adding or removing lots from a published Contract Notice require launching a new competition.
However, withdrawal is possible at lot level as an exceptional case. Each lot has its own lifecycle:
+
--
- When a lot is withdrawn, the contracting authority must publish a Result notice (CAN) with “No winner chosen” and the non-award reason.
- If the withdrawal occurs before the tender deadline, the withdrawn lot may then be removed from subsequent Contract Notice versions through a Change notice, provided the withdrawal is clearly documented in the Change section and references the Result notice.
- If no further CN changes are made, no additional publication is required beyond the Result notice.
--
+
This exception is only acceptable if the overall scope of the procedure remains coherent and is not significantly altered.
+
*🛈* From SDK 1.14 onward, *BT-634 must not be used in Contract Notices*.
The earlier practice of using BT-634 to flag withdrawn or cancelled lots is discontinued.
+
Please note that all notices successfully submitted will be published.
The publication of a notice cannot be cancelled once it has entered the Publishing stage.
Users may stop publication only while the notice is still in Submitted status, before the daily export to TED.



When creating a Change notice, how should it reference and update previous notices?::

A Change notice is a reproduction of its parent notice with all data from that notice included and updated as needed. It also contains a dedicated Change section (BG-9) that describes the modifications made to the previously published version.
+
Each Change notice applies to a single parent notice, identified by NoticeID and VersionID, and is linked using BT-758 Change Notice Version Identifier. Within the Change section, individual modifications target specific sections using BT-13716 Change Previous Notice Section Identifier, describe the reason through BT-140 Change Reason Code, and may include additional explanation in BT-141 Change Description.
+
A Change notice must therefore:
+
--
- Contain all data from the parent notice, with the applied changes integrated.
- Include the Change section explaining what has changed since the last published version.
- Reference any affected sections or lots precisely.
--
+
If the procurement documents have changed, use BT-718 Procurement Documents Change Indicator and BT-719 Procurement Documents Change Date; describe those updates in BT-141 Change Description.
+
Each new Change notice should represent the consolidated state of the notice: it already incorporates previous changes and describes only the latest modifications.
+
If multiple changes are needed in sequence:
+
--
- Submit and publish the first Change notice before sending the next, or
- Combine all pending changes into a single valid Change notice.
--
+
Change notices may only be applied to published notices; if a notice has not yet been published, it can simply be stopped and resubmitted instead of issuing a Change.
+
The VersionID is used only for internal versioning before publication. Once a notice is published, its VersionID is final. Change notices always have their own identifiers and VersionIDs, independent of the parent notice’s version.
+
If a Change notice cancels its parent notice, no further Change can be applied to that cancelled notice.
+
For withdrawn lots, the withdrawal must first be recorded in a Result notice with “No winner chosen.” The withdrawal may then be documented in the Change section of a later CN, referencing that Result notice, if the withdrawn lot is removed before the tender deadline.
...
For more information, see the relevant https://docs.ted.europa.eu/eforms-common/FAQ/index.html#_apis_and_web_services[section]. 
...
Every working day, according to the OJ S https://ted.europa.eu/en/release-calendar[release calendar], 
...
+
See also the developer documentation about https://docs.ted.europa.eu/eforms/latest/versioning.html[SDK versioning].  
+
...
is the decision to group almost all the organisation information in one section. 
+
...
+
The lifespan of the various SDK versions is documented on the link:https://docs.ted.europa.eu/eforms-common/active-versions/index.html[Active SDK versions] page, however, 
eSenders should always consult the https://docs.ted.europa.eu/eforms-common/active-versions/index.html#version-range[information provided by the TED API]. 
+
See also the https://docs.ted.europa.eu/api/latest/developer-operations.html[Developer Operations API].
...
Please read the link:https://docs.ted.europa.eu/api/latest/index.html[TED API docs]. 
...
eSenders, however, are required to be familiar with the link:https://enotices2.ted.europa.eu/terms[“Terms of service”].
+
The environments available are Production and https://docs.ted.europa.eu/eforms-common/preview/index.html[Preview]. 
+
The Preview environment will be available indefinitely so that users can test validation of notices against new https://docs.ted.europa.eu/eforms-common/active-versions/index.html[versions of the SDK]. 
The latter will first be implemented in Preview environment before going into Production. 
+
The Central Validation Service (CVS) is remotely available via the https://docs.ted.europa.eu/api/latest/validate.html[Validation API]
so that you can check the validity of eForms notices before submitting them for publication. 
...
to generate a list of eSenders using eForms.
...
+
Swagger URLs for the API are available on the https://docs.ted.europa.eu/api/latest/publish.html[Publication API] page.
...
Every notice must include at least one lot.
+
If the procedure covers only one lot, that lot is a technical lot, and its identifier must be LOT-0000.
+
Numbering does not need to be sequential; gaps are allowed (for example, LOT-0001, LOT-0003, LOT-0008).
If the notice contains multiple real lots, no technical lot (LOT-0000) should be used.
+
To reference a lot across notices or in later stages of the procedure, use the Internal Identifier (BT-22) rather than the technical lot ID. This ensures traceability, even if technical IDs differ between notices.
+
The same principle applies to parts in planning notices:
+
--
- A PIN with a single part should use PAR-0000 as its identifier.
--
+
...
+
--
- BT-195 Unpublished Identifier – indicates which field is unpublished via a codelist (file: non-publication-identifier.gc)
...
+
Each republication keeps the same Notice ID but receives a new Publication ID.
The Unpublished Accessibility Date must be between 2 days and 10 years after the Dispatch Date (BT-05).
+
Example:
If the framework value (BT-118) is unpublished, it will appear as “unpublished” on TED until the date in BT-198 is reached, after which the notice is republished with the value visible.
...
From SDK 1.14 onward, BT-634 must not be used in Contract (Competition) Notices.
The earlier practice of using it to flag withdrawn or cancelled lots during the competition phase is discontinued.
+
BT-634 is now used only in Result notices (CANs) to indicate that a procedure or lot will be relaunched after being cancelled or not awarded.
This allows statistical tracking and transparency while keeping Contract Notices focused on active competitions.
...
When calling the https://docs.ted.europa.eu/api/latest/validate.html[Validation API], the “text” element in the validation report will be returned in the language you passed as a parameter to your request. 
...
If a field is mandatory but left empty or if a code choice is mandatory but not chosen, will the notice be rejected and not published?::  
...
There are several additional business rules that check the validity of the content of different fields, i.e., combinations of
fields.
+
There are rules that will block (reject) the submission of eForms notices, particularly in cases that
violate or contradict the Procurement Directives. All these rules are implemented using Schematron.
+
Notice validation is automated through the Central Validation System. See https://docs.ted.europa.eu/api/latest/validate.html[Validation API].
+
Human validation will only be done for notices that have a https://docs.ted.europa.eu/eforms/latest/reference/business-rules/index.html#_lawfulness[“lawfulness”] warning. 
