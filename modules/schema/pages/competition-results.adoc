...
higher review body instance, a review requests must receive a new identifier.
...
a|
An identifier of the review request(s) that led to this decision or a 
review decision that is being appealed by this review request. Review 
decision which was initiated by the review body (“ex officio”) are not 
...
a|
An identifier of one or more to of a sections within this notice. The 
...
It may only have a value from the “_review-information-type_” codelist and helps 
specify whether the information is about a Review Request (“req”) or a Review 
Decision (“dec”).
...
It is used for internal reference in the notice (e.g. to identify the review 
...
This repeatable field allows to identify the sections of the notice subject to 
...
This repeatable field allows to identify the organisations that request the Review 
...
organisation and is only optional.
...
This multi-lingual field is mandatory and contains the title of the Review Request 
or Review Decision

*Review Description (BT-789-Review)*

This multi-lingual field is mandatory and contains the description or any other 
...
made the decision (for a Review Decision). It should contain a code of the https://op.europa.eu/web/eu-vocabularies/dataset/-/resource?uri=http://publications.europa.eu/resource/dataset/review-body-type[review-body-type] code list.
...
This non-repeatable field identifies the type of made decision and should 
...
confirmed, then this field should be used to list all the confirmed irregularities.
...
This optional non-repeatable field allows to specify a web address for the documents of the 
Review Request or Review Decision.

*Review Request Fee (BT-795-Review)*

This optional field is only allowed for a Review Decision and allows to specify the fee paid 
for lodging the review request.

*Review Request Withdrawn (BT-796-Review)*

This indicator is only allowed for a Review Decision and allows to specify whether the 
...
This optional multi-lingual field may only be specified when the Request has been withdrawn. 
It allows to provide details on the background reason that lead to the Request withdrawal.
...
<efac:Appeals> 
	<efac:AppealInformation>
		...
		<cbc:Title languageID="FRE">Demande de révision de l'attribution du lot 2</cbc:Title>
		<!-- Review Description (BT-789) -->
		<cbc:Description languageID="ENG">Given the deadline for ...</cbc:Description>
		<cbc:Description languageID="FRE">Etant donnée la date limite ...</cbc:Description>
		<!-- Review URL (BT-794) -->
		...
		<efbc:AppealStageCode listName="review-type">req</efbc:AppealStageCode>
		...
		<efac:AppealedItem>
			<cbc:ID schemeName="section">RES-0003</cbc:ID>
		</efac:AppealedItem>
		<efac:AppealedItem>
			<cbc:ID schemeName="section">LOT-0002</cbc:ID>
		</efac:AppealedItem>
	...
	<efac:AppealInformation>
		...
		<cbc:Title languageID="FRE">Rejet de la demande de révision ...</cbc:Title>
		<!-- Review Description (BT-789) -->
		<cbc:Description languageID="ENG">The Review requested by ...</cbc:Description>
		<cbc:Description languageID="FRE">La demande de révision initiée par ...</cbc:Description>
		<!-- Review Request Fee (BT-795) -->
		<cbc:FeeAmount currencyID="EUR">9999</cbc:FeeAmount>
		<!-- Review Decision URL (BT-794) -->
		...
		<!-- Review Request or Decision (BT-783) -->
		<efbc:AppealStageCode listName="review-type">dec</efbc:AppealStageCode>
		<!-- Previous Review Identifier (BT-785) -->
		<efbc:PreviousReviewIdentifier>ABCD/9983 - ghs/JE</efbc:PreviousReviewIdentifier>
		<efac:AppealIrregularity>
			<efbc:IrregularityTypeCode listName="irregularity-type">insuf-timl</efbc:IrregularityTypeCode>
		</efac:AppealIrregularity>
		<!-- Review Notice Section Identifier (BT-786) -->
