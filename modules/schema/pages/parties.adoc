...
|*M* |*O* |*EM* |*M* |*CM* |*CM* |*M* |*CM* |*M* |*M* |*O*

|*_Touchpoint (Buyer)_* |*—* |*—* |*—* |*—* |*—* |*—* |*EM* |*O* |*—*
|*M* |*—* |*M* |*O* |*EM* |*M* |*CM* |*CM* |*M* |*CM* |*CM* |*CM* |*O*
...
An indicator may be used to specify whether the legal person is a natural
person or not (BT-633).

The Winner Listed (BT-746) "_efbc:ListedOnRegulatedMarketIndicator_"
element is not allowed for Non-Tenderer, Non-Buyer organizations.

The "_efac:Company_" element groups all the elements related to the legal
...
.Non-tenderer, Non-buyer components
...
The Winner Owner Nationality (BT-706) is expressed using the 
"_cbc:NationalityID_" element. Values must come from the 
...
and a single AddressLine for 3^rd^ and subsequent lines
...
|*Winner Size (BT-165)* ^|O a|
[source,xpath,subs=attributes]
----
efac:Company/efbc:CompanySizeCode
----
|Only for tenderers, winners and subcontractors; Code from the 
https://op.europa.eu/web/eu-vocabularies/at-dataset/-/resource/dataset/economic-operator-size[economic-operator-size] 
codelist
...
In the following tables, for contexts and procedures where the roles/subroles
followed by the *(#)* are allowed, the corresponding roles/subroles have to be 
included and their associated organisations identified.
...
| <|Review Information Providing Organization |M |O |O |EM |M |EM |EM |M
|O |M |M |EM |O |O |O
...
17+|(+++#+++) roles/subroles followed by *(#)* must be included and their associated 
organisations identified everywhere the Procedure and Context allow it.
|===
...
information item in the column header, is expressed using the codes detailed in
the legend below:
...
Organization* |*Documents provider organization* |*Review Information
...
12+|(+++#+++) roles/subroles followed by *(#)* must be included and their associated 
organisations identified everywhere the Procedure and Context allow it.
|===

In eForms some roles and subroles have been identified and described
<<parties-R1>>. The following sections help mapping these with the appropriate
schema components.
...
----
/*/cac:ProcurementProjectLot{zwsp}/cac:TenderingTerms{zwsp}/cac:AppealTerms{zwsp}/cac:MediationParty
----

|*Review organization (#)* |Organization dealing with review requests.
a|[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot{zwsp}/cac:TenderingTerms{zwsp}/cac:AppealTerms{zwsp}/cac:AppealReceiverParty
----
...
[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot{zwsp}/cac:TenderingTerms{zwsp}/cac:FiscalLegislationDocumentReference{zwsp}/cac:Attachment{zwsp}/cac:ExternalReference{zwsp}/cbc:URI
----
...
[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot{zwsp}/cac:TenderingTerms{zwsp}/cac:FiscalLegislationDocumentReference{zwsp}/cac:IssuerParty
----
...
[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot{zwsp}/cac:TenderingTerms{zwsp}/cac:EnvironmentalLegislationDocumentReference{zwsp}/cac:Attachment{zwsp}/cac:ExternalReference{zwsp}/cbc:URI
----
...
[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot{zwsp}/cac:TenderingTerms{zwsp}/cac:EnvironmentalLegislationDocumentReference{zwsp}/cac:IssuerParty
----
...
[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot{zwsp}/cac:TenderingTerms{zwsp}/cac:EmploymentLegislationDocumentReference{zwsp}/cac:Attachment{zwsp}/cac:ExternalReference{zwsp}/cbc:URI
----
...
[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot{zwsp}/cac:TenderingTerms{zwsp}/cac:EmploymentLegislationDocumentReference{zwsp}/cac:IssuerParty
----
...
3+|(+++#+++) roles/subroles followed by *(#)* must be included and their associated 
organisations identified everywhere the Procedure and Context allow it.
|===
...
a|[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot/cac:TenderingTerms{zwsp}/cac:AdditionalInformationParty
----
...
a|[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot/cac:TenderingTerms{zwsp}/cac:DocumentProviderParty
----
...
a|[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot/cac:TenderingTerms{zwsp}/cac:AppealTerms{zwsp}/cac:AppealInformationParty
----
...
a|[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot/cac:TenderingTerms{zwsp}/cac:TenderRecipientParty
----
...
a|[source,xpath,subs=attributes]
----
/*/cac:ProcurementProjectLot/cac:TenderingTerms{zwsp}/cac:TenderEvaluationParty
----
...
|Review Information Providing Organisation |S | | |1 | |1 | |1
| | |1 | | |1 | | |1 | | |1 | | |1 | | | |
...
found in xref:competition-results.adoc#tenderingPartySection[Tendering Party].
...
* Has been selected as part of the winning Tenders (Tenders associated to a Contract, 
i.e. Tender for which the value of its OPT-321-Tender field equals the value of a BT-3202-Contract 
field of an existing Contract. 
.).
...
| *Notice Subtype*  |*1* |*2* |*3* |*4* |*5* |*6* |*7* |*8* |*9*
...
|Group leader (Buyer) |EM |EM |EM |EM |EM |EM |EM |EM |EM |EM |EM |EM |EM |EM |EM |EM |EM |EM |EM
...
|Review Information Providing Organization |-- |-- |-- |CM |CM |O |CM |O |O
...
41+|(+++#+++) roles/subroles followed by *(#)* must be included and their associated 
organisations identified everywhere the Procedure and Context allow it.
|===
...
For the eForms technical implementation, it has been decided to keep the normalized approach for 
organisations to prevent unnecessary repetitions and their associated inconsistency risks.
The denormalized approach of the UBL Standard has been kept and reduced to its minimum (i.e. use 
of references to the organisation or touchpoint involved) as it is associated to a context which 
helps differentiate various roles/subroles.
The references to the organisation or its touchpoint is performed  using OPT-300-xxx or 
OPT-301-xxxx fields. As a rule of thumb, OPT-300-xxxx is associated to an organisation (i.e. 
identifier of the form ORG-XXXX) while OPT-301-xxxx is associated to an organisation or a touchoint 
(i.e. identifier of the form ORG-XXXX and TPO-XXXX respectively). Please note that exceptions may 
occur with business rules clarifications while the field has already been defined.
...
Any Contracting Party must contain one and only one reference to an organisation acting 
as Buyer (OPT-300-Procedure-Buyer). When multiple buyers must explicitly be identified, 
then multiple "_cac:ContractingParty_" elements must be used.
...
Tenders and multiple Tenders could be submitted by the same Tendering
Party,

* There may also be multiple winning Tenders (e.g. Framework
Agreement),

Therefore, when identifying a specific winner of a Lot, it is important to
identify the Tender and check that it is a winning one.

=== Contract Signatory

The Contract Signatory(-ies) may be found in the "_efac:SettledContract_"
...
* [[[parties-R1,R1]]] **eForms Policy Implementation Handbook**, _European
