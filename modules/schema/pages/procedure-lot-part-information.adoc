...
|*EU Funds Financing Identifier (BT-5010)* |xref:fundsInformationSection[xrefstyle=short] |-- |-- |*?* |-- |-- |*?* |-- |-- 
a|
...
|*EU Funds Programme (BT-7220)* |xref:fundsInformationSection[xrefstyle=short] |-- |-- |*?* |-- |-- |*?* |-- |-- 
a|
[source,xpath,subs=attributes]
----
ext:UBLExtensions{zwsp}/ext:UBLExtension{zwsp}/ext:fundsInformation{zwsp}/efext:EformsExtension{zwsp}/efac:Funding{zwsp}/cbc:FundingProgramCode
----

|*EU Funds Details (BT-6140)* |xref:selectionCriteriaSection[xrefstyle=short] |-- |-- |*?* |-- |-- |*?* |-- |-- 
a|
[source,xpath,subs=attributes]
----
...
[[fundsInformationSection]]
=== Selection criteria

When EU Funds (BT-60) indicates that EU FUnds are involved in the procurement, 
EU Funds Informattion (BG-614) may be given, this covers:

* EU Funds Programme (BT-7220),

* EU Funds Financing Identifier (BT-5010), and

* EU Funds Details (BT-6160).


Details may only be provided once, either of the both, BT-7220 or BT-5010 exist.
The content of EU Funds Programme must match one of the codes of the 
https://op.europa.eu/web/eu-vocabularies/at-dataset/-/resource/dataset/eu-programme[eu-programme] codelist.
The content of the EU Funds Financing Identifier shall be the most concrete possible 
(e.g. grant agreement number, national identifier or project acronym).
...
"_cbc:LanguageID_" element has to be used with values from the
...
Both are within the same "_cac:TendererQualificationRequest_" element and express 
requirements on the tenderer itself.
...
Use of Non Disclosure Agreement for the contract execution may be encoded with the
...
and either the 'true' or 'false' value. When the value is set to 'true', further details may be
 provided as text in the cbc:Decription element.
...
Prize information may be used for various Planning and Competition notices. While it is necessary for 
Design Contest competition notices, it may also be useful for other notices when the procedure is about 
"Innovation Partnership" or "Competitive Dialogue".
...
Previous Notice (OPP-090) may be used to associate a notice to one or 
more that just preceded. It is not intended to replace:
...
It is only foreseen for cases where no other option to refer to a previous notice exist. It may be useful when a Procedure already started in TEDXML and goes on in eForms, this Business Term allows then to associate the notices despite the absence of Procedure ID in TEDXML.
A notice is referred to using either of the two identifiers:

* its notice identifier (UUID) concatenated with its version identifier (e.g. <UUUID>-<vv>), or 

* its Notice Publication ID (XXXXXX-YYYY).

Previous Notice (OPP-090) applies to the whole notice, and not just a single lot.
It may also be repeated to point to multiple previous notices when needed.
...
<cac:NoticeDocumentReference>
    <cbc:ID schemeName="notice-id-ref">33b03807-58ce-4eb2-aac2-25702d0ad624-06</cbc:ID>
</cac:NoticeDocumentReference>
----
