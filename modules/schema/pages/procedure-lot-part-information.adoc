...
more notices that just preceded. It is not intended to replace:
...
It is only foreseen for cases where no other option to refer to a previous notice exist. 
It may be useful when a Procedure already started in TEDXML and goes on in eForms, this 
Business Term allows then to associate the notices despite the absence of Procedure ID in TEDXML.
