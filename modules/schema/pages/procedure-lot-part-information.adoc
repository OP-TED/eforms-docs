...
more immediately preceding notices. It is not intended to replace:
...
It is only foreseen for cases where no other option to refer to a previous notice exists. 
It may be useful when some notices for a Procedure have been published in TEDXML, 
and it is desired to publish following notices of the Procedure in eForms. This 
Business Term allows the notices to be associated despite the absence of Procedure ID in TEDXML.

A notice is referred to using either of the two identifiers:

* its notice identifier (UUID) concatenated with its version identifier (e.g. <UUID>-<vv>), or 
