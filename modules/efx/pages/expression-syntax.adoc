...
== Predicates 
A field or node reference can be complimented with a predicate that restricts the `XML elements` it matches. 

Just like in XPath, EFX predicates are logical (boolean) EFX expressions included in square brackets.
...
Duration expressions allow to manipulate durations including:
