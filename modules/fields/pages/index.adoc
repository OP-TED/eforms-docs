...
=== Forbidden and mandatory constraints

These constraints indicate for which notice types and under which conditions the field is forbidden or mandatory.

If no specific forbidden or mandatory constraint exists for a notice type, the field is considered optional: it can be present but is not required.
...
If the constraint does not depend on anything other that the notice type, the condition is indicated as "true".
...
The forbidden or mandatory constraint of a field can also depend on more complex conditions:
...
----
    {
      "noticeTypes" : [ "2", "5" ],
      "forbidden" : "!(field('BT-740-Procedure-Buyer')=='cont-ent')" // <1>
    },
    {
      "noticeTypes" : [ "1", "2", "4", "5" ],
      "mandatory" : "true" // <2>
...
----
<1> This field is forbidden for notices "2" and "5" if the field BT-740-Procedure-Buyer does not contain the value 'cont-ent'.
<2> If allowed, this field is mandatory for notices "1", "2", "4" and "5".

NOTE: The forbidden and mandatory constraints must be considered together: a field must first be allowed (not forbidden) before it can be considered mandatory.
...
The value of the `forbidden` and `mandatory` properties is a string representing a boolean expression.
