...
`schemeNames`:: Applicable only to fields of type `id-ref`. Provides an array of valid scheme names that this `id-ref` field can reference.
...
`id-ref`:: string representing a reference to an identifier (has an associated `idSchemes` and `schemeNames`)
